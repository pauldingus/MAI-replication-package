...
tw  (rbar lower_grow upper_grow monthmid if growing==1, barw(32) fc("$color2*0.8") lw(0)) ///
	...
	xlabel(`=td(15jan2018)' "1/2018" `=td(15jul2018)' "7/18" `=td(15jan2019)' "1/19" `=td(15jul2019)' "7/19" `=td(15jan2020)' "1/20" `=td(15jul2020)' "7/20" ) name(rain, replace) graphregion(color(white) margin(none)) title(" ", pos(10)) ylabel(0(200)200, angle(0)) legend(off) ///
	text(410 `labgrow'  "Growing ", color("$color2") placement(w)) /// 
	text(410 `labharv'  "Harvest", color("$color3") placement(e))  fysize(20) ///
	title("{bf:b}", color(white)  size(*0.8)) plotregion(lstyle(none) )
local bw=20
local degree=0
...
tw  (lpolyci activity_harmonized_2018 date if mktday==1 , bw(`bw') degree(`degree') lcolor("$color1") fcolor("$color1%25") alw(0) lw(*2)) /// 
	...
	xlabel(`=td(15jan2018)' " " `=td(15jul2018)' " " `=td(15jan2019)' " " `=td(15jul2019)' " " `=td(15jan2020)' " " `=td(15jul2020)' " " , ticks labsize(0)) xscale(range(`=td(15jan2018)'(100)`=td(15jul2020)')) ///
	ylabel(0 50 100, angle(0)) ///
	title("{bf:b}", pos(10) color(black) size(*0.8)) ///
